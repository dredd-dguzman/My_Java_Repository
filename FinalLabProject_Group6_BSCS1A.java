        ...
        switch (command) {
            ...
            case "=" -> { // Evaluate the expression and display the result
                try {
                    ...
                    // Add spaces around operators for proper parsing by the eval method's regex
                    ...
                    formattedExpr = formattedExpr.replaceAll(" {2,}", " ").trim(); // Remove any extra spaces
            ...
            case "+/-" -> { // Toggle the sign of the current number
                if (currentInput.length() > 0 && currentInput.charAt(0) == '-') {
                    currentInput.deleteCharAt(0); // Remove minus sign if present
                } else {
                    currentInput.insert(0, "-"); // Add minus sign
                }
            }
            case "√" -> unary(Math::sqrt); // Perform square root
            case "x²" -> unary(a -> a * a); // Perform squaring
            case "1/x" -> unary(a -> 1 / a); // Perform reciprocal (1 divided by x)
            default -> currentInput.append(command); // For digits and other operators, append to current input
        }
        ...
        return new Object() {
            ...
            double parseFactor() {
                ...
                if (eat('(')) { // Handle parenthesized expressions
                    ...
                    eat(')'); // Expect closing parenthesis
